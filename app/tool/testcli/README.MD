...
*这个是testing/lich的二进制工具版本（Go请直接使用库版本：git.atmatrix.org/k12/zero/pkg/testing/lich)*
...
```shell
go get -u git.atmatrix.org/k12/zero/tool/testcli
$GOPATH/bin/testcli -h
```
#### Method 2. Build with Go
```shell
cd git.atmatrix.org/k12/zero/tool/testcli
go build -o $GOPATH/bin/testcli
$GOPATH/bin/testcli -h
```
#### Method 3. Import with Kratos pkg
```Go
import "git.atmatrix.org/k12/zero/pkg/testing/lich"
```
...
- Step1. 在 Dao|Service 层中的 TestMain 单测主入口中，import  "git.atmatrix.org/k12/zero/pkg/testing/lich" 引入testcli工具的go库版本。
